...
function ProductPage() {
    ...
    const handleSendEmail = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setAiResponse(result.message || "Failed to send");
            }
        } catch (error: any) {
            console.error('Send email error:', error);
            setAiResponse("Failed to send email");
        } finally {
            setIsSending(false);
        }
    };
    ...
    const inboxData = realEmails.length > 0 ? realEmails.map((e: any) => ({
        id: e.id,
    ...
    `}>
                ...
                <div className={`px-5 py-6 shrink-0 z-20 flex flex-col gap-4 ${darkMode ? 'bg-[#050505]/95' : 'bg-[#FAFAFA]/95'} backdrop-blur-md`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className={`h-8 w-8 rounded-md border border-dashed flex items-center justify-center shrink-0 transition-colors ${darkMode ? 'border-white/10 text-white/30 hover:border-red-600/30 hover:text-red-600' : 'border-black/10 text-black/30 hover:border-red-600/30 hover:text-red-600'}`}
                            title="Add Email Account"
                        >
                            <span className="text-xs">+</span>
                        </button>
                        {zohoConfigured && (
                            <div className="ml-2 px-2 py-1 rounded-md bg-blue-600/10 border border-blue-600/20">
                                <span className="text-[8px] font-bold uppercase tracking-wider text-blue-600">Zoho</span>
                            </div>
                        )}
                    </div>
                </div>
      ...
      `}>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8 no-scrollbar pb-48">
                        ...
                        {activeView === 'inbox' && !selectedMailId && !composeMode && (
                            <div className="space-y-3 max-w-2xl mx-auto">
                                <h2 className={`text-3xl font-light tracking-tight mb-6 ${darkMode ? 'text-white' : 'text-black'}`}>
                                    Inbox {isLoadingEmails && <span className="text-xs opacity-50">syncing...</span>}
                                </h2>
                                {inboxData.map(mail => (
            ...
            <SettingsModal
                ...
                onThemeModeChange={(mode: ThemeMode) => setDarkMode(mode === 'dark')}
            />
            ...
            {!isHubOpen && (
                <button
                    ...
                    className="fixed bottom-24 right-6 z-[70] w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 group bg-red-600 text-white"
