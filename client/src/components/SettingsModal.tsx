...
// InputField component defined OUTSIDE render to prevent focus loss
const InputField: React.FC<{
    ...
    isDark: boolean;
    id?: string;
}> = ({ label, placeholder, type = "text", value, onChange, isDark, id }) => (
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, themeMode, onThemeModeChange }) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setAccounts(data.map((acc: any) => ({
                    ...acc,
                    avatarColor: acc.provider === 'gmail' ? 'bg-red-500' : acc.provider === 'outlook' ? 'bg-blue-500' : 'bg-indigo-500',
                    lastSync: 'Just now',
                    quota: 0,
                    status: 'active'
                })));
    ...
    const handleProviderChange = (provider: string) => {
        let defaults: Partial<AccountData> = {};
        switch (provider) {
            case 'Gmail': defaults = { imapHost: 'imap.gmail.com', smtpHost: 'smtp.gmail.com', imapPort: '993', smtpPort: '587' }; break;
            case 'Outlook': defaults = { imapHost: 'outlook.office365.com', smtpHost: 'smtp.office365.com', imapPort: '993', smtpPort: '587' }; break;
            case 'iCloud': defaults = { imapHost: 'imap.mail.me.com', smtpHost: 'smtp.mail.me.com', imapPort: '993', smtpPort: '587' }; break;
            case 'Zoho': defaults = { imapHost: 'imappro.zoho.com', smtpHost: 'smtp.zoho.com', imapPort: '993', smtpPort: '465' }; break;
            case 'Titan': defaults = { imapHost: 'imap.titan.email', smtpHost: 'smtp.titan.email', imapPort: '993', smtpPort: '587' }; break;
            default: defaults = { imapHost: '', smtpHost: '', imapPort: '993', smtpPort: '587' }; break;
        }
    ...
    const handleEmailChange = useCallback((val: string) => {
        setEditingAccount(prev => ({ ...prev, email: val }));
    }, []);

    const handlePasswordChange = useCallback((val: string) => {
        setEditingAccount(prev => ({ ...prev, password: val }));
    }, []);

    const handleImapHostChange = useCallback((val: string) => {
        setEditingAccount(prev => ({ ...prev, imapHost: val }));
    }, []);

    const handleImapPortChange = useCallback((val: string) => {
        setEditingAccount(prev => ({ ...prev, imapPort: val }));
    }, []);

    const handleSmtpHostChange = useCallback((val: string) => {
        setEditingAccount(prev => ({ ...prev, smtpHost: val }));
    }, []);

    const handleSmtpPortChange = useCallback((val: string) => {
        setEditingAccount(prev => ({ ...prev, smtpPort: val }));
    }, []);
        ...
        try {
            const res = await fetch('/api/email/test-connection', {
                ...
                body: JSON.stringify({
                    ...
                    provider: editingAccount.provider?.toLowerCase()
                })
            });
            ...
            if (res.ok && data.success) {
                ...
                setTestStatus('error');
                setTestMessage(data.error || data.message || 'Connection failed');
            }
        } catch (error: any) {
            setTestStatus('error');
    ...
    const handleSaveAccount = async () => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                localStorage.setItem('userEmail', editingAccount.email);
                ...
                setTestMessage(data.error || 'Failed to save account');
            }
        } catch (error: any) {
            setTestMessage(error.message || 'Failed to save');
        } finally {
            setIsSaving(false);
        }
    };
                                    ...
                                    <div className="space-y-8">
                                        ...
                                        {accounts.length === 0 ? (
                                            <div className={`p-8 rounded-2xl border text-center ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                                                <Mail size={48} className={`mx-auto mb-4 ${isDark ? 'text-slate-600' : 'text-slate-400'}`} />
                                                <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                                                    No email accounts connected yet.
                                                </p>
                                                <p className={`text-xs mt-2 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
                                                    Click "Add Account" to connect your Gmail, Outlook, or other email.
                                                </p>
                                            </div>
                                        ) : (
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 overflow-y-auto custom-scrollbar pr-2 pb-4">
                                                ...
                                                {editingAccount.provider === 'Gmail' && (
                                                    <div className={`p-4 rounded-xl border ${isDark ? 'bg-amber-500/10 border-amber-500/20' : 'bg-amber-50 border-amber-100'}`}>
                                                        <p className={`text-xs ${isDark ? 'text-amber-300' : 'text-amber-800'}`}>
                                                            ...
                                                            Go to <a href="https://myaccount.google.com/apppasswords" target="_blank" rel="noopener noreferrer" className="underline">Google App Passwords</a> to create one.
                                                        </p>
                                                    </div>
                                                )}
                                            ...
                                            <div className={`p-6 rounded-2xl border flex flex-col h-full ${isDark ? 'bg-white/5 border-white/5' : 'bg-white/40 border-slate-200'}`}>
                                                ...
                                                <div className="pt-4 border-t border-dashed border-white/10">
                                                    {testMessage && (
                                                        <div className={`mb-3 p-3 rounded-lg text-xs ${testStatus === 'error' ? 'bg-red-500/20 text-red-300' : 'bg-emerald-500/20 text-emerald-300'}`}>
                                                            {testMessage}
                                                        </div>
                                                    )}
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={testStatus === 'testing' || !editingAccount.email || !editingAccount.password}
                                                        className={`w-full py-4 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all disabled:opacity-50 disabled:cursor-not-allowed
                                                    ${testStatus === 'success'
                                                ...
                                                `}
                                                    ...
                                                    {testStatus === 'success' && (
                                                        <button
                                                            ...
                                                            disabled={isSaving}
                                                            className="w-full mt-3 py-4 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-3 animate-in fade-in slide-in-from-bottom-2 bg-[#121214] text-white border border-white/20 hover:bg-black disabled:opacity-50"
                                                        >
                                                            {isSaving ? (
                                                                <RefreshCw size={16} className="animate-spin" />
                                                            ) : (
                                                            ...
                                                            {isSaving ? 'Saving...' : 'Save Account Config'}
                                                        </button>
                                                    )}
                                                </div>
                                            </div>
                            ...
                            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <InputField label="Language" value="English (US)" isDark={isDark} />
                                        <InputField label="Time Zone" value="(UTC+04:00) Abu Dhabi, Muscat" isDark={isDark} />
                                    </div>
                                    <div className="mt-6">
