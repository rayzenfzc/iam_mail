...
export function BiometricLogin({ onLogin, isDarkMode, toggleTheme }: BiometricLoginProps) {
      ...
      `}>
        ...
        <div className="px-10 pt-16 md:pt-14 space-y-6">
          <div className="font-light text-6xl md:text-7xl tracking-tighter leading-none dark:text-white transition-all select-none text-center">
            <span className="text-slate-300 dark:text-slate-600">i.</span>
            <span className="text-slate-900 dark:text-slate-100 font-medium">M</span>
          </div>
        ...
        <div className="flex flex-col items-center gap-10 flex-1 justify-center px-10">
          <button 
            ...
            className={`relative group p-14 rounded-[56px] transition-all duration-1000 ${
              isAuthenticating ? 'scale-90 opacity-40 grayscale' : 'hover:scale-105 active:scale-95'
            }`}
            data-testid="button-biometric-auth"
          >
            <div className={`absolute inset-0 rounded-[56px] border-[2px] border-dashed border-primary/30 transition-opacity duration-500 ${isAuthenticating ? 'animate-spin opacity-100' : 'opacity-20 group-hover:opacity-100'}`} style={{ animationDuration: isAuthenticating ? '2s' : '20s' }} />
            <div className="absolute inset-4 rounded-[40px] border border-primary/10" />
            
            <Fingerprint 
              size={88} 
              strokeWidth={0.5} 
              className={`transition-all duration-700 ${isAuthenticating ? 'text-primary' : 'text-slate-300 dark:text-slate-700 group-hover:text-primary group-hover:scale-110'}`}
            />
          </button>

          <div className="space-y-6 w-full">
            <h2 className="text-2xl md:text-3xl thin-title text-slate-900 dark:text-slate-100 uppercase tracking-tight leading-tight select-none text-center">
            ...
            <div className="w-full space-y-5">
              ...
              <div className="flex flex-col items-center pt-2">
                ...
                <div className="mt-4 w-12 h-[1.5px] bg-slate-100 dark:bg-slate-900" />
              </div>
            </div>
          </div>
        </div>
