...
const App: React.FC = () => {
  ...
  const themeStyles = useMemo(() => {
    ...
    if (themeMode === 'dark_pro') {
      return {
        bgColor: '#000000', // Pure Black
        dotColor: '#262626' // Neutral 800
      };
    }
    if (themeMode === 'pure') {
      return {
        bgColor: '#FFFFFF', // Pure White
        dotColor: '#E2E8F0' // Slate 200
      };
    }
    if (themeMode === 'dawn') {
      return {
        bgColor: '#FDFBF7', // Warm Off-White (Stone 50/Paper)
        dotColor: '#E7E5E4' // Stone 200
      };
    }
    if (themeMode === 'light_pro') {
      return {
        bgColor: '#F1F5F9', // Slate 100 - Tech White
        dotColor: '#94A3B8' // Slate 400 - High Contrast Dots
      };
    }
    // Default Light (Silver)
    return {
      bgColor: '#E5E5E5',
      dotColor: '#CCCCCC'
    };
  ...
  const cycleTheme = () => {
    setThemeMode(prev => {
      if (prev === 'pure') return 'dawn';
      if (prev === 'dawn') return 'light';
      if (prev === 'light') return 'light_pro';
      if (prev === 'light_pro') return 'dark';
      if (prev === 'dark') return 'midnight';
      if (prev === 'midnight') return 'dark_pro';
      return 'pure';
    });
  };
