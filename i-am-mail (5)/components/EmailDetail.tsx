...
import { FileText, Download, Reply, Forward, Trash2, X, ChevronLeft, ChevronRight, CheckCircle2, MoreVertical, Paperclip, Sparkles, BrainCircuit, Wand2, ReplyAll } from 'lucide-react';
...
const EmailDetail: React.FC<EmailDetailProps> = ({ email, isDark, onClose, onReply, onForward }) => {
  ...
  const handleSummarize = async () => {
      if (!email || !process.env.API_KEY) return;
      setIsSummarizing(true);
      try {
          const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
          const result = await ai.models.generateContent({
              model: 'gemini-3-flash-preview',
              contents: `Summarize this email in 3 bullet points. Focus on action items. \n\n ${email.body}`
          });
  ...
  return (
    <div className="flex flex-col h-full bg-transparent relative z-0">
        ...
        {/* HEADLESS HEADER - FLOATS OUTSIDE CARD */}
        <div className="px-8 pt-10 pb-6 flex items-center gap-6 shrink-0 bg-transparent z-20">
             <div className="flex items-center gap-6 flex-1">
                 {/* Tall Vertical Bar */}
                 <div className={`w-[3px] h-12 ${isDark ? 'bg-white' : 'bg-slate-900'}`}></div>
                 
                 <div className="flex flex-col justify-between h-12 py-0.5">
                     <span className={`text-[0.6rem] font-bold uppercase tracking-[0.2em] opacity-60 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                        INCOMING_SECURE
                     </span>
                 ...
                 {totalMessages > 1 && (
                      <div className="flex items-center gap-2 mr-4">
                          <button 
                              ...
                              className={`p-3 rounded-full disabled:opacity-20 ${isDark ? 'bg-white/5 hover:bg-white/10' : 'bg-white hover:bg-slate-100 shadow-sm'}`}
                          >
                              <ChevronLeft size={16} />
                          </button>
        ...
        {/* Content - Floating Glass Card with Padded Container */}
        <div className="flex-1 overflow-hidden relative z-10 px-4 pb-4">
             <div className={`
                h-full w-full rounded-[2.5rem] overflow-y-auto custom-scrollbar p-8 lg:p-12 border transition-all relative
                ${isDark ? 'bg-[#121214]/40 backdrop-blur-md border-white/10' : 'bg-white/60 backdrop-blur-md border-white/40 shadow-xl shadow-slate-900/5'}
             `}>
                  ...
                  <div className={`flex flex-col gap-6 mb-8 border-b border-dashed pb-8 ${isDark ? 'border-white/10' : 'border-slate-300/50'}`}>
                       ...
                       {/* Subject Line Large */}
                       <div className={`text-3xl font-black leading-tight tracking-tight mt-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                           {email.subject}
                       </div>
                  ...
                  {/* Intelligence Panel */}
                  <div className={`hidden lg:flex mb-8 p-6 rounded-[1.5rem] border flex-col gap-4 ${isDark ? 'bg-indigo-500/5 border-indigo-500/10' : 'bg-indigo-50/30 border-indigo-100/50'}`}>
                      <div className="flex justify-between items-center">
                          ...
                          {!summary && (
                              <button 
                                ...
                                className={`text-[0.6rem] px-3 py-1.5 rounded-full border flex items-center gap-2 transition-all
                                    ${isDark ? 'border-white/10 hover:bg-white/10' : 'bg-white/50 border-slate-200 hover:shadow-sm'}
                                `}
                              >
                                {isSummarizing ? <span className="animate-pulse">Thinking...</span> : <><Wand2 size={12}/> Generate Brief</>}
                              </button>
                          )}
                      </div>
                      {summary && (
