...
import { 
  ...
  RefreshCw, Edit2, AlertCircle, Save, Lock, ArrowRight, Database, Cloud, Droplet, Coffee, Zap, Activity
} from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, themeMode, onThemeModeChange }) => {
  ...
  const isDark = themeMode === 'dark' || themeMode === 'midnight' || themeMode === 'dark_pro';
  ...
  let bgStyle = { backgroundColor: '#E5E5E5', dotColor: '#CCCCCC' };
  if (themeMode === 'midnight') bgStyle = { backgroundColor: '#020617', dotColor: '#1e293b' };
  if (themeMode === 'dark') bgStyle = { backgroundColor: '#0A0A0B', dotColor: '#1F2937' };
  if (themeMode === 'dark_pro') bgStyle = { backgroundColor: '#000000', dotColor: '#262626' };
  if (themeMode === 'pure') bgStyle = { backgroundColor: '#FFFFFF', dotColor: '#E2E8F0' };
  if (themeMode === 'dawn') bgStyle = { backgroundColor: '#FDFBF7', dotColor: '#E7E5E4' };
  if (themeMode === 'light_pro') bgStyle = { backgroundColor: '#F1F5F9', dotColor: '#94A3B8' };
    ...
    <div className="fixed inset-0 z-[110] flex flex-col h-full w-full overflow-hidden">
      ...
      <div 
        ...
        style={{
            ...
            backgroundImage: `radial-gradient(${bgStyle.dotColor} 1px, transparent 1px)`,
                    ...
                    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div>
                             ...
                             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                                <button
                                    onClick={() => onThemeModeChange('pure')}
                                    className={`p-6 rounded-[1.5rem] border-2 text-left transition-all duration-300 group ${themeMode === 'pure' ? 'border-indigo-400 bg-white shadow-xl' : 'border-transparent bg-white/60 hover:bg-white'}`}
                                >
                                    <Droplet size={24} className="mb-4 text-indigo-500" />
                                    <div className="font-black text-xs text-slate-900 uppercase tracking-widest group-hover:text-indigo-600">Pure</div>
                                    <div className="text-[10px] text-slate-400 mt-1">High Contrast White</div>
                                </button>

                                <button
                                    onClick={() => onThemeModeChange('dawn')}
                                    className={`p-6 rounded-[1.5rem] border-2 text-left transition-all duration-300 group ${themeMode === 'dawn' ? 'border-amber-400 bg-[#FDFBF7] shadow-xl' : 'border-transparent bg-[#FDFBF7]/60 hover:bg-[#FDFBF7]'}`}
                                >
                                    <Coffee size={24} className="mb-4 text-amber-600" />
                                    <div className="font-black text-xs text-slate-800 uppercase tracking-widest group-hover:text-amber-700">Dawn</div>
                                    <div className="text-[10px] text-slate-400 mt-1">Warm Paper</div>
                                </button>

                                <button
                                    onClick={() => onThemeModeChange('light')}
                                    className={`p-6 rounded-[1.5rem] border-2 text-left transition-all duration-300 group ${themeMode === 'light' ? 'border-slate-900 bg-[#E5E5E5] shadow-xl' : 'border-transparent bg-[#E5E5E5]/60 hover:bg-[#E5E5E5]'}`}
                                >
                                    <Sun size={24} className="mb-4 text-slate-900" />
                                    <div className="font-black text-xs text-slate-900 uppercase tracking-widest">Silver</div>
                                    <div className="text-[10px] text-slate-500 mt-1">Standard Light</div>
                                </button>
                                
                                <button
                                    onClick={() => onThemeModeChange('light_pro')}
                                    className={`p-6 rounded-[1.5rem] border-2 text-left transition-all duration-300 group ${themeMode === 'light_pro' ? 'border-indigo-600 bg-[#F1F5F9] shadow-xl' : 'border-transparent bg-[#F1F5F9]/60 hover:bg-[#F1F5F9]'}`}
                                >
                                    <Zap size={24} className="mb-4 text-indigo-600" />
                                    <div className="font-black text-xs text-slate-900 uppercase tracking-widest">i.M Light Pro</div>
                                    <div className="text-[10px] text-slate-500 mt-1">Tech White</div>
                                </button>

                                <button
                                    onClick={() => onThemeModeChange('dark')}
                                    className={`p-6 rounded-[1.5rem] border-2 text-left transition-all duration-300 group ${themeMode === 'dark' ? 'border-indigo-500 bg-slate-900 shadow-xl' : 'border-transparent bg-slate-900/60 hover:bg-slate-900'}`}
                                >
                                    <Moon size={24} className="mb-4 text-white" />
                                    <div className="font-black text-xs text-white uppercase tracking-widest">Dark</div>
                                    <div className="text-[10px] text-slate-400 mt-1">Deep Grey</div>
                                </button>

                                <button
                                    onClick={() => onThemeModeChange('midnight')}
                                    className={`p-6 rounded-[1.5rem] border-2 text-left transition-all duration-300 group ${themeMode === 'midnight' ? 'border-indigo-400 bg-slate-950 shadow-xl' : 'border-transparent bg-slate-950/60 hover:bg-slate-950'}`}
                                >
                                    <Cloud size={24} className="mb-4 text-indigo-400" />
                                    <div className="font-black text-xs text-indigo-100 uppercase tracking-widest">Midnight</div>
                                    <div className="text-[10px] text-slate-500 mt-1">Navy Blue</div>
                                </button>

                                <button
                                    onClick={() => onThemeModeChange('dark_pro')}
                                    className={`p-6 rounded-[1.5rem] border-2 text-left transition-all duration-300 group ${themeMode === 'dark_pro' ? 'border-white bg-black shadow-xl shadow-white/5' : 'border-transparent bg-black/60 hover:bg-black'}`}
                                >
                                    <Activity size={24} className="mb-4 text-white" />
                                    <div className="font-black text-xs text-white uppercase tracking-widest">i.M Dark Pro</div>
                                    <div className="text-[10px] text-slate-400 mt-1">True Black</div>
                                </button>
                             </div>
                             <div className="space-y-4">
